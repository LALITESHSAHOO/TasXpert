...
public class AttachmentTask extends AsyncTask<Void, Void, Attachment> {
    ...
    protected Attachment doInBackground(Void... params) {
        return StorageHelper.createAttachmentFromUri(EventsRefresher.getAppContext(), uri);
    }
    ...
    protected void onPostExecute(Attachment mAttachment) {
        if (isAlive()) {
            ...
            if (mAttachment != null) {
                StorageHelper.delete(EventsRefresher.getAppContext(), mAttachment.getUri().getPath());
            }
        }
    }
