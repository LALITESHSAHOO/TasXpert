...
public class TextHelper {
    ...
    public static Spanned[] parseTitleAndContent(Context mContext, Note note) {
        ...
        Spanned newcontent = Html.fromHtml(Html.fromHtml(content).toString());

        String formattedText = newcontent.toString();



        if (note.getTitle().length() > 0) {
            titleText = note.getTitle();
            contentText = limit(formattedText, 0, CONTENT_SUBSTRING_LENGTH, false, true);
        } else {
            titleText = limit(formattedText, 0, TITLE_SUBSTRING_OF_CONTENT_LIMIT, true, false);
            contentText = limit(formattedText.replace(titleText, "").trim(), 0, CONTENT_SUBSTRING_LENGTH, false, false);
        }
