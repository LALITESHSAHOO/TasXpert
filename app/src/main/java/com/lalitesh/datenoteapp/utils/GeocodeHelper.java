...
public class GeocodeHelper implements LocationListener {
	...
	private GeocodeHelper() {
		instance = this;
		locationManager = (LocationManager) EventsRefresher.getAppContext().getSystemService(Context.LOCATION_SERVICE);
	}
	...
	public static void getLocation(OnGeoUtilResultListener onGeoUtilResultListener) {
		SmartLocation.LocationControl bod = SmartLocation.with(EventsRefresher.getAppContext()).location(new
				LocationGooglePlayServicesWithFallbackProvider(EventsRefresher.getAppContext())).config(LocationParams
				.NAVIGATION).oneFix();
	...
	public static void stop() {
		SmartLocation.with(EventsRefresher.getAppContext()).location().stop();
		if (Geocoder.isPresent()) {
			SmartLocation.with(EventsRefresher.getAppContext()).geocoding().stop();
		}
	}
	...
	public static void getAddressFromCoordinates(Location location,
		...
		if (!Geocoder.isPresent()) {
			...
			SmartLocation.with(EventsRefresher.getAppContext()).geocoding().reverse(location, (location1, list) -> {
	...
	public static void getCoordinatesFromAddress(String address, final OnGeoUtilResultListener
			listener) {
		SmartLocation.with(EventsRefresher.getAppContext()).geocoding().direct(address, (name, results) -> {
			if (results.size() > 0) {
